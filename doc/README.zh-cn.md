...
如果需要更省流量或更高的下载速度，请加上`--mode 0`，`--mode 0`模式会牺牲一点点的延迟(默认最多8ms，可调)换取更低的流量消耗和更高的吞吐率。
...
对于`mode 0`模式，为了达到不用关注mtu的效果，`-f x:y`里的x必须>=2。 `mode 0`模式中，x=1的情况下编码器会丧失调整mtu的能力，x=1和`mode 0`一起使用是不推荐的用法。
